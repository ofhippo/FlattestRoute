...
function updateRoutes() {
	...
	//$(".travel-info").show();
	routes.forEach(parseSlope);
//	newPath(path, distance.value);

}

function parseSlope(route, index) {
	...
	var plotElevation = function (elevations, status) {
		...
		for (var i=0; i<len; i++) {
			elevArr.push(elevations[i].elevation);
		}
		calcSlope(elevArr, distance);
	};
	var calcSlope = function (elevArr, distance) {
		var slopeArr = [];
		var len = elevArr.length;
		for (var i=0; i<len-1; i++) {
			var slope = (elevArr[i+1]-elevArr[i])/distance;
			slopeArr.push(slope);
		}
		chooseRoute(slopeArr);
	};
	var chooseRoute = function(slopeArr) {
		var total = slopeArr.reduce(function(a,b){
			return a + b;
		});
		console.log(total);
	}
	elevator.getElevationAlongPath(pathRequest, plotElevation.bind({distance:distance}));
}
